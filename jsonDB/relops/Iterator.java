...
public abstract class Iterator {
	...
	{
		//TODO
		String fileName = q.getFromTable();
		Table table = Parser.parseTable(fileName);
		Schema s = table.getSchema();
		List<String> selectFields = q.getSelectFields();
		List<List<Predicate>> wherePredicates = q.getWherePredicates();


		//start with FileScan
		FileScan child = new FileScan(table);

		//if no selection is done
		if (wherePredicates.isEmpty()) {
			Projection ok = new Projection(child, selectFields);
			...
			Selection select = new Selection(child,wherePredicates.get(0));

			select.setSchema(child.getSchema());
			System.out.println("Size"+wherePredicates.size());
			for (int i = 1; i < wherePredicates.size(); i++) {


				select = new Selection(select,wherePredicates.get(i));
				select.setSchema(child.getSchema());


			}

			Projection ok = new Projection(select,selectFields);
			ok.setSchema(Projection.project(select.getSchema(),selectFields));
